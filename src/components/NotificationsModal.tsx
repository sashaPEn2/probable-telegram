...
export const NotificationsModal: React.FC<NotificationsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-blue-200 flex flex-col max-h-[85vh]">
        
        {/* Заголовок */}
        <div className="bg-gradient-to-r from-[#0a2a5e] to-blue-900 p-6 text-white flex items-center justify-between flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-200 font-mono mt-0.5">Личный кабинет исследователя</p>
            </div>
          </div>
        ...
        {notifications.length > 0 && (
          <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between text-xs font-semibold text-slate-600 flex-shrink-0">
            ...
            <div className="flex space-x-3">
              <button onClick={() => onMarkRead()} className="text-blue-700 hover:underline flex items-center space-x-1">
                <CheckCheck className="w-3.5 h-3.5" /> <span>Прочитать все</span>
              </button>
              <button onClick={onClearAll} className="text-red-600 hover:underline flex items-center space-x-1">
            ...
            notifications.map(n => (
              <div 
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-start space-x-3 ${
                  !n.is_read ? 'bg-blue-50/70 border-blue-200 shadow-sm' : 'bg-white border-slate-200 opacity-75'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h4 className="font-bold text-xs sm:text-sm text-[#0a2a5e] truncate pr-2">{n.title}</h4>
                ...
                {!n.is_read && (
                  <div className="w-2 h-2 rounded-full bg-[#d4af37] self-center flex-shrink-0"></div>
                )}
              </div>
            ))
          )}
        ...
        <div className="p-4 bg-slate-100 border-t border-slate-200 text-center flex-shrink-0">
          <button onClick={onClose} className="px-6 py-2 bg-[#0a2a5e] text-white font-bold text-xs rounded-xl hover:bg-blue-900 shadow">
            Закрыть окно
          </button>
        </div>

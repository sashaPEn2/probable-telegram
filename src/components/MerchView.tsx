...
export const MerchView: React.FC<MerchViewProps> = ({ db, user, onRefresh }) => {
  ...
  const getStatusBadge = (status: MerchOrder['status']) => {
    switch (status) {
      case 'pending': return (
        <span className="flex items-center space-x-1 px-2 py-1 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase">
          <Clock className="w-3 h-3" />
          <span>Ожидает</span>
        </span>
      );
      case 'ready': return (
        <span className="flex items-center space-x-1 px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-[10px] font-bold uppercase">
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-black text-[#0a2a5e] dark:text-blue-200 mb-2 italic">SNO MERCH STORE</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="text-2xl font-black text-[#0a2a5e] dark:text-blue-100">{stats.ratingPoints}</div>
            ...
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                  Сувениры выдаются в <span className="font-bold">деканате ФЭМ</span> (Корпус 4, каб. 314) у <span className="text-[#0a2a5e] dark:text-blue-300 font-bold">заместителя декана</span> по научной работе.
                </p>
              </div>
              <div className="flex items-start space-x-3">
        ...
        <div className="lg:col-span-3 space-y-8">
            ...
            {db.merch.map((item) => (
              ...
              >
                <div className="aspect-[16/9] bg-slate-100 dark:bg-slate-800 flex items-center justify-center relative">
                  ...
                  <div className="absolute top-4 right-4 px-3 py-1 bg-white/90 dark:bg-slate-900/90 backdrop-blur rounded-full text-xs font-black text-amber-600 border border-amber-200 dark:border-amber-800">
                    {item.points} pts
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="font-black text-[#0a2a5e] dark:text-blue-200 mb-2 group-hover:text-blue-600 transition-colors">
                    {item.name}
                  </h3>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-black transition-all ${
                        ...
                        : 'bg-[#0a2a5e] text-white hover:bg-blue-800 shadow-lg shadow-blue-900/20 active:scale-95'
                      }`}
                    >
          ...
          {userOrders.length > 0 && (
            <div className="mt-12">
              <h2 className="text-xl font-black text-[#0a2a5e] dark:text-blue-200 mb-6 flex items-center space-x-2">
                    ...
                    {userOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((order) => (
                      <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="text-sm font-bold text-[#0a2a5e] dark:text-blue-200">{order.itemName}</div>

...
export const SnilView: React.FC<SnilViewProps> = ({
  ...
  return (
    <div className="space-y-12 pb-24">
      ...
      {/* Header Section */}
      <div className="relative bg-[#052e16] rounded-[2.5rem] p-8 sm:p-14 overflow-hidden shadow-2xl border border-[#10b981]/30">
        <div className="absolute top-0 right-0 w-80 h-80 bg-[#10b981]/10 rounded-full -mr-32 -mt-32 blur-3xl animate-pulse"></div>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-emerald-500/10 rounded-full -ml-16 -mb-16 blur-3xl"></div>
        
        <div className="relative z-10 space-y-6">
          <div className="flex items-center space-x-3 text-[#10b981] text-xs font-black uppercase tracking-[0.3em] font-mono">
          ...
          <p className="text-emerald-100/70 text-sm sm:text-lg max-w-3xl leading-relaxed font-medium">
      ...
      <motion.div 
        ...
        className="bg-blue-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800 rounded-3xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-300"
      >
        <button
          onClick={() => setIsJoinSectionOpen(!isJoinSectionOpen)}
          className="w-full p-6 sm:p-8 flex items-center justify-between text-left focus:outline-none hover:bg-emerald-100/20 dark:hover:bg-emerald-900/20 transition-colors"
        >
          <div className="flex items-center space-x-4">
            <div className="w-12 h-12 rounded-2xl bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm flex-shrink-0 border border-emerald-200 dark:border-emerald-700">
              <FileText className="w-6 h-6 text-[#052e16] dark:text-emerald-400" />
            </div>
            <div>
              <h3 className="text-lg sm:text-xl font-black text-[#052e16] dark:text-white uppercase tracking-tight">Как вступить в СНИЛ ФЭМ?</h3>
          ...
          <motion.div
            ...
            className="text-slate-400 dark:text-slate-500 hover:text-[#052e16] dark:hover:text-white p-1 rounded-lg"
          >
        ...
        <AnimatePresence initial={false}>
          {isJoinSectionOpen && (
            <motion.div
              ...
              className="border-t border-emerald-100 dark:border-emerald-800/80"
            >
              <div className="p-6 sm:p-8 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-2 p-5 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs">
                    <div className="flex items-center space-x-2 text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">
                      <span className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center font-mono">1</span>
                  ...
                  <div className="space-y-2 p-5 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs">
                    <div className="flex items-center space-x-2 text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">
                      <span className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center font-mono">2</span>
                  ...
                  <div className="space-y-2 p-5 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs">
                    <div className="flex items-center space-x-2 text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">
                      <span className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center font-mono">3</span>
            ...
            {db.announcements.map((ann) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`p-6 rounded-3xl border ${ann.is_urgent ? 'bg-blue-50/50 dark:bg-emerald-900/10 border-emerald-200 dark:border-amber-800' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm'}`}
                >
                  ...
                  <h4 className="text-lg font-black text-[#052e16] dark:text-white mb-2 leading-tight">{ann.title}</h4>
      ...
      {selectedSnilId && snils.length < allSnils.length && (
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-blue-50/50 dark:bg-blue-950/20 px-6 py-4 rounded-3xl border border-emerald-100 dark:border-emerald-900/60"
        >
          <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
            Показана выбранная лаборатория: <span className="text-[#052e16] dark:text-emerald-400 font-extrabold">СНИЛ {snils[0]?.name}</span>
          </p>
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
        {snils.map((snil) => {
          ...
          return (
            ...
            >
              {/* Card Decoration */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 dark:bg-emerald-900/10 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-700"></div>

              <div className="p-8 sm:p-10 pb-4 relative z-10">
                <div className="flex items-start justify-between mb-6">
                  <div className="w-14 h-14 bg-blue-50 dark:bg-emerald-900/30 rounded-2xl flex items-center justify-center text-[#052e16] dark:text-emerald-300 border border-emerald-100 dark:border-emerald-800 group-hover:rotate-6 transition-transform">
                    <Microscope className="w-7 h-7" />
                  </div>
                  {snil.is_best_snil_nominee && (
                    <div className="flex items-center space-x-2 bg-blue-50/50 dark:bg-emerald-900/30 px-4 py-2 rounded-xl border border-emerald-200 dark:border-amber-800 shadow-sm">
                      <Trophy className="w-4 h-4 text-[#10b981]" />
                      <span className="text-[10px] font-black uppercase tracking-widest text-[#10b981]">Лучшая СНИЛ 2026</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-black text-[#052e16] dark:text-white leading-tight">
                    СНИЛ {snil.name}
                  </h3>
                </div>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {snil.research_directions.map((dir, i) => (
                      <span key={i} className="text-[10px] font-bold px-3 py-1.5 bg-blue-50/50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 rounded-lg border border-emerald-100/50 dark:border-emerald-900/50">
                        {dir}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      {snil.achievements.map((ach, i) => (
                        <div key={i} className="flex items-start space-x-2.5 text-xs text-slate-600 dark:text-slate-400 bg-emerald-600/5 dark:bg-emerald-600/10 p-3 rounded-2xl border border-emerald-500/10 dark:border-emerald-500/20 shadow-xs">
              ...
              <div className="p-8 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 mt-auto flex flex-col sm:flex-row gap-3">
                {isMember ? (
                  ...
                  <div className="w-full py-4 rounded-2xl bg-blue-50/50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center space-x-2 border border-emerald-200 dark:border-amber-900">
                  ...
                  <button
                    ...
                    disabled={!user}
                    className="w-full py-4 rounded-2xl bg-[#052e16] dark:bg-emerald-600 text-[#10b981] dark:text-white font-black text-xs uppercase tracking-[0.2em] shadow-lg hover:brightness-110 transition-all flex items-center justify-center space-x-2 active:scale-95 disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed"
                  >
